...
app.get(
  ...
  async (c: Context) => {
    ...
    const data: Base = {
      ...
      image: `${url.origin}/${chain}/image/${id}/${sn}`,
      animation_url: withUrlParam(base.animation_url, 'hash', hash),
...
app.get('/:chain/image/:id/:sn', async (c: Context) => {
  ...
  const hash = await hexOf(sn)
  const link = base.animation_url.replace('ipfs://', IPFS_GW)
  const url = withUrlParam(link, 'hash', hash)
